...
void D_AddSDMFile (const char *file); //GHK
void D_AddWildSDMFile (const char *pattern); //GHK
...
//GHK
EXTERN_CVAR (Int, sd_global_hiscores)
EXTERN_CVAR (Bool, sd_custom_replacements)
EXTERN_CVAR (Int, sd_noinfight)
EXTERN_CVAR (Int, sd_pistolstart)
...
CVAR (Bool, sd_first_run, true, CVAR_ARCHIVE); //GHK Used for prompting user to personalise name & pwd
...
//called on startup
bool sd_c_docmd_ok = true;


//ghk
bool sd_do_default_monsters = false;

//ghk
bool sd_startannounce_didonce=false;

//GHK used to disallow hi scores, when console is used etc...
bool sd_remote_hiscores_ok = true;

//GHK: used to check whether a custom SD config is loaded (like sdcustom.ini)
bool sd_sdcustom_ini = false;

int sd_dmflags = 0;

//determines whether offline, online or online-hardcore. (0,1,2+)
int sd_online_level=1;

//ghk  determines whether infighting and infighting damage is disabled
int sd_notarget=1;

//ghk  determines whether we are in pistol start mode or not
int sd_piststart=1;

//GHK anti-cheat: Used for the 'global' md5 value (the .exe & gzscoredoom.pk3)
const char *sd_global_md5="\0";

//GHK anti-cheat:  used for patch (deh, bex) patch md5 value
const char *sd_wads_md5="\0"; //called in args.considerpatches

//GHK anti-cheat:  SD-ADDONPACK hash sent seperately now
const char *sd_addonpack_md5="\0"; //called in args.considerpatches

//GHK anti-cheat: sdgldefs.pk3  hash sent seperately
const char *sd_gldefs_md5="\0"; //called in args.considerpatches

//GHK anti-cheat:  sd configvalues hash
const char *sd_configvalues_md5="\0"; //called in args.considerpatches

//GHK FR SDCUTOM.INI drag n drop
const char *sd_global_sdmini_path="\0";

std::string sdConfigValues("");
...
CVAR (Flag, compat_forcenoblockmonst,compatflags, COMPATF_FORCEBLOCKMONST);
CVAR (Flag, compat_memorialfix,compatflags, COMPATF_MEMORIALFIX);
...
//ghk for drag n drop sdcustom.ini
void D_AddSDMFile (const char *file)
{
	if (file == NULL)
	{
		return;
	...
	if (!FileExists (file))
	{
		const char *f = BaseFileSearch (file, ".ini");
		if (f == NULL)
		{
			Printf ("Can't find '%s'\n", file);
			return;
...
//ghk
void D_AddWildSDMFile (const char *value)
{
	const char *wadfile = BaseFileSearch (value, ".ini");

	if (wadfile != NULL)
	{
		D_AddSDMFile (wadfile);
	}
	else
	{ // Try pattern matching
		...
		if (handle != ((void *)-1))
		{
			do
			{
				if (sep == NULL)
				{
					D_AddSDMFile (I_FindName (&findstate));
				}
				else
				{
	...
	if (files->NumArgs() > 0)
	{
		...
		for (i = 0; i < files->NumArgs(); ++i)
		{
			if ( (f = BaseFileSearch (files->GetArg (i), ".deh")) ){
				//GHK Add deh file md5 check
				//need to add similar to gethiscore, sethiscore code too
				std::string hash2(sd_wads_md5);
				hash2 += md5.getHashFromFile(f);
				p+=sprintf (buffer+p, "%s",hash2.c_str());

				DoDehPatch (f, false);
			}else if ( (f = BaseFileSearch (files->GetArg (i), ".bex")) ){
				//GHK Add bex file md5 check
				//need to add similar to gethiscore, sethiscore code too
				std::string hash2(sd_wads_md5);
				hash2 += md5.getHashFromFile(f);
				p+=sprintf (buffer+p, "%s",hash2.c_str());
		...
		sd_wads_md5=buffer; //GHK
		//Need to set up rules for applying patches
		//like -deh a.deh b.bex per wad which uses them
		//Printf ("SD got patch hash code %s\n",sd_wads_md5);
...
void D_DoomMain (void)
{
	...
	if (!(gameinfo.flags & GI_SHAREWARE))
	{
		...
		if (home)
		{
			//GHK, Change this to gzscoredoom?
			sprintf (file, "%s%s.gzdoom/skins", home,
				home[strlen(home)-1] == '/' ? "" : "/");
			D_AddDirectory (file);
		}
	...
	if(execFiles->NumArgs()>0) //GHK disallow autoexec files with remote scoring
	...
	DArgs *files3 = Args.GatherFiles (NULL, ".ini", false);
	...
	if (files3->NumArgs() > 0){
		for (int i = 0; i < files3->NumArgs(); i++)
		{
			...
			D_AddWildSDMFile (files3->GetArg (i));
		}
	}
	...
	//ghk: no need now.
	//if(Args.CheckValue("+dmflags")||Args.CheckValue("+dmflags2")){
		//GHK. dont allow dmflags
		//sd_remote_hiscores_ok=false;
	//}
	...
	//if (gameinfo.gametype == GAME_Doom)
	{
			...
			GameConfig->SetSection ("Doom.DefaultDehacked"))
		{
			...
			while (GameConfig->NextInSection (key, value))
			{
				...
				{
					//GHK Add Deh/bex check here too.
		...
		DoSDDehPatch (NULL, true);	// GHK See if there's a SDHACKED patch in a PWAD
									// Mainly for ADD-ON PACK support for wads like PL2.WAD
									// that have the DEHACKED lump in the wad
		FinishDehPatch ();			// Create replacements for dehacked pickups
	}
	FActorInfo::StaticSetActorNums ();


		//GHK: Grab remote hi scores here, if console var set, and cheating not
	//detected.
	//Printf ("Checking global hi score table...\n");
	//if md5 has not found...
	//Printf ("Wad configuration not supported for global hi scores.\n");
	//Printf ("Reverting to local hi scores.\n");

	//GHK: Set flag so that any call to C_DoCommand (c_dispatch.cpp) is
	//not allowed when remote scoring is enabled.

	//*MD5 CHECKSUM needs to consider deh, bex from args.considerpatches too.*
	//Need to clarify deh/bex order for md5 checksum, per wad.

	//if(sd_global_hiscores){
		//md5wrapper md5;

		//std::string hash2 md5.getHashFromFile("gzdoom.exe");
		//hash2+=md5.getHashFromFile(Wads.GetWadFullName(0));

		Printf ("Generating ScoreDoom hash codes... \n");
		md5wrapper md5;

		std::string hash2(md5.getHashFromFile("gzscoredoom.exe"));

		hash2+=md5.getHashFromFile(Wads.GetWadFullName(0));


		//Printf ("SD got total global hash code %s\n",hash2.c_str());

		hash2+=SDMD5SEED; //add private seed from sd_md5seed.H
		sd_global_md5 = hash2.c_str();

		hash2 = md5.getHashFromString(hash2.c_str());
		sd_global_md5 = hash2.c_str();
		//Printf ("SD got global hash code %s\n",sd_global_md5);

		//sd_c_docmd_ok = false;

	//}
	std::string wadstr(sd_wads_md5);
	std::string addonpackstr(sd_addonpack_md5);
	std::string sdgldefstr(sd_gldefs_md5);
	int i;
	const char *arg;
	//now 1=2, since BBAx16 is in gzcoredoom.pk3
	for (i = 2; (arg = Wads.GetWadName (i)) != NULL; ++i)
	{
		//if (debugfile) fprintf (debugfile, "wadnames:%s",arg);
		if(strcmp(arg, "SD-ADDONPACK.WAD")!=0&&strcmp(arg, "sdgldefs.pk3")!=0){
			wadstr += md5.getHashFromFile(Wads.GetWadFullName(i));
		}else{
			if(strcmp(arg, "SD-ADDONPACK.WAD")==0){
				addonpackstr = md5.getHashFromFile(Wads.GetWadFullName(i));
				sd_addonpack_md5=addonpackstr.c_str();
			}else if(strcmp(arg, "sdgldefs.pk3")==0){
				sdgldefstr = md5.getHashFromFile(Wads.GetWadFullName(i));
				sd_gldefs_md5=sdgldefstr.c_str();
			}

		}
		//Printf ("SD got hash code %i%s\n",i,wadstr.c_str());

	}

	if(*(sd_addonpack_md5)==0)
		sd_addonpack_md5="0";

	if(*(sd_gldefs_md5)==0)
		sd_gldefs_md5="0";

	sd_wads_md5 = wadstr.c_str();


	//Printf ("SD got total wad hash code %s\n",wadstr.c_str());

	//get the real hash code now, generated from the sum of
	//hash codes, contained in global.
	wadstr = md5.getHashFromString(wadstr.c_str());
	sd_wads_md5 = wadstr.c_str();
	//Printf ("Wad Hash Code Generated from Wads & Patches: %s",sd_wads_md5);
	//Used for viewing generated hash code:
	...
	//GHK Get any User ini custom monster replacement settings

	if(sd_custom_replacements){

		GameConfigSDM = new FGameConfigFile("SDMOVERRIDE"); //look for sdcustom.ini
		if(!GameConfigSDM->HaveSections()){
			GameConfig->GetSDUserReplacements(); //just use regular user ini

		}else{
			sd_sdcustom_ini=true;
			//sd_remote_hiscores_ok=false; <--not necessarily, let the server check the provided hash config.
			GameConfig->GetSDUserReplacementsSDM(GameConfigSDM); //override with sdcustom.ini
			//delete GameConfigSDM;
			//GameConfigSDM = NULL;
		}
	}else{
		sd_configvalues_md5="0"; //set to "0" since no hash generated otherwise in this case
	}
	...
	//GHK: No remote hi scores in multiplayer!!
	if(multiplayer){
		sd_remote_hiscores_ok=false;
	}
	...
	if(sd_first_run){
		...
		if(sd_global_hiscores&&!sd_remote_hiscores_ok){ //&&!multiplayer){
			//Printf ("**FOR DEMOS...**\n");
			Printf ("**ONLINE HI SCORING DISABLED FOR FIRST RUN**\n");
			Printf ("**PLEASE RESTART SCOREDOOM FOR ONLINE HI SCORING**\n");
		}
		sd_first_run=!sd_first_run;
	}

	//ghk finally make sure to set sd_remote_hiscores_ok to false
	//if sd_global_hiscores is false at startup
	//dont allow players to set online scoring after startup.
		if(!sd_global_hiscores)
			sd_remote_hiscores_ok=false;

	//GHK
	//Printf ("Setting dmflags and dmflags2 to default(0)\n");
		//dmflags=dmflags2=0;
	if(!multiplayer){
		sd_online_level=sd_global_hiscores;

		sd_notarget=sd_noinfight;

		sd_piststart=sd_pistolstart;
	}else{
		//dont apply these flags in multiplayer at all
		sd_online_level=0;
	...
	if (files->NumArgs() > 0)
	{
		...
		if(sd_global_hiscores&&!sd_remote_hiscores_ok){ //&&!multiplayer){
			//Printf ("**FOR DEMOS...**\n");
			Printf ("**ONLINE HI SCORING DISABLED FOR DEMO PLAYBACK**\n");
		}
