...
//GHK
bool CheckBossDeathSD (AActor *actor)
{
	int i;
	...
	while ( (other = iterator.Next ()) )
	{
		...
		if (other != actor &&
			(other->health > 0 || (other->flags & MF_ICECORPSE))
			&& other->SDParentPClass == actor->SDParentPClass)
			//&& other->GetClass()->SDParentClass == actor->GetClass()->SDParentClass)
		{ // Found a living boss
		  // [RH] Frozen bosses don't count as dead until they shatter
			//if (debugfile) fprintf (debugfile, "SDPARENTCLASS foundboss: %s ",actor->SDParentPClass->TypeName.GetChars());
			//if (debugfile) fprintf (debugfile, "SDPARENTCLASS foundboss: %s ",other->SDParentPClass->TypeName.GetChars());
...
//GHK 
/*bool FoundSDBossType(FName BossType, mytypeinfo){
	
	if(BossType=NAME_Fatso){


	}else if(BossType=NAME_Arachnotron){

	}else if(BossType=NAME_SpiderMastermind){

	}else if(BossType=NAME_Cyberdemon){
		if(mytypeinfo=PClass::FindClass ("Apprentice")||mytypeinfo=PClass::FindClass ("Avatar")||
			newtypeinfo = PClass::FindClass ("HellSmith1")||newtypeinfo = PClass::FindClass ("Thamuz"))

	}else if(BossType=NAME_BaronOfHell){
...
void A_BossDeath (AActor *actor)
{
	...
	//if (debugfile) fprintf (debugfile, "mytype1: %s",mytype.GetChars());

	// Ugh...
	FName type = actor->GetClass()->ActorInfo->GetReplacee()->Class->TypeName;

	//if (debugfile) fprintf (debugfile, "type1: %s",type.GetChars());
	...
	if (gameinfo.gametype == GAME_Doom){
		//GHK Now grab ahold of the SD Replacee's typename, if it exists.
		//It always should from StaticSpawn
		//PClass *mytypeinfo = actor->GetClass()->SDParentClass;
		FName SDParentPClass = actor->SDParentPClass;
	
		//if (debugfile) fprintf (debugfile, "mytypeinfo0: %s",actor->GetClass()->TypeName.GetChars());
		
		//if (debugfile) fprintf (debugfile, "mytypeinfo1: %s",SDParentPClass.GetChars());
		

		if(SDParentPClass){
			
			//if (debugfile) fprintf (debugfile, "ghk1c?%d",1);
			
			//type=mytypeinfo->TypeName;
			//type=SDParentPClass;

			//GHK, Major Hack to get HellsBattery cool ending to work.
			//Now also add SDBossChainEnder, for chained bosses

			if(strcmp(type.GetChars(),"HellsBatteryGore")==0||strcmp(type.GetChars(),"SDBossChainEnder")==0){
				if(level.flags & LEVEL_CYBORGSPECIAL){
					type = NAME_Cyberdemon;
					actor->SDParentPClass=NAME_Cyberdemon;
				}else if(level.flags & LEVEL_SPIDERSPECIAL){
					type = NAME_SpiderMastermind;
					actor->SDParentPClass=NAME_SpiderMastermind;
				}else if(level.flags & LEVEL_BRUISERSPECIAL){
					type = NAME_BaronOfHell;
					actor->SDParentPClass=NAME_BaronOfHell;
				}

			}else if(strcmp(type.GetChars(),"SDBossChainEnderS")==0){
				
				if(level.flags & LEVEL_SPIDERSPECIAL){
					type = NAME_SpiderMastermind;
					actor->SDParentPClass=NAME_SpiderMastermind;
				}

			}else if(strcmp(type.GetChars(),"SDBossChainEnderB")==0){
				if(level.flags & LEVEL_BRUISERSPECIAL){
					type = NAME_BaronOfHell;
					actor->SDParentPClass=NAME_BaronOfHell;
				}
			...
			//if (debugfile) fprintf (debugfile, "SDCLASS YES: %s ",actor->GetClass()->TypeName.GetChars());
			//if (debugfile) fprintf (debugfile, "SDPARENTCLASS YES: %s ",actor->SDParentPClass.GetChars());
	
		}else{
			//if (debugfile) fprintf (debugfile, "ghk1c?%d",2);
			//if (debugfile) fprintf (debugfile, "SDPARENTCLASS NOT: %s ",type.GetChars());
	
		}
	}
	
	//if (debugfile) fprintf (debugfile, "SDPARENTCLASS NAME_Fatso: %d ",type == NAME_Fatso);
