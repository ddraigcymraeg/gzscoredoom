...
CVAR (Bool,		freelook,		true,	CVAR_GLOBALCONFIG|CVAR_ARCHIVE)		// Always mlook? //GHK YES!
		...
		case ga_savegame:
			{
				...
				if(sd_global_hiscores&&sd_remote_hiscores_ok&!multiplayer){
					if(sd_online_level<2){ //GHK dont allow non-auto saves in online-hardcore
						G_DoSaveGame (true);
					}else{
						Printf("Player saves not allowed in Hardcore mode.");
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i])
		{
				...
				!(gametic&31) && ((gametic>>5)&(MAXPLAYERS-1)) == i )
			{
				...
				//GHK
				if(!multiplayer&&sd_global_hiscores&&sd_remote_hiscores_ok){
...
void G_PlayerFinishLevel (int player, EFinishLevelType mode, bool resetinventory)
{
	...
	//GHK: TRY HERE to reset the player's level_score
	//Make sure on saving hi scores that this is cool
	//keep level score if moving within a HUB
	if(mode != FINISH_SameHub){
		//if (debugfile) fprintf (debugfile, "ghkPLEVEL1SCORE%d",p->level_score);
		p->level_score=0;
	}
	//}else if (mode == FINISH_SameHub){
		//p->wad_score=0; //ghk: explanation: treat a HUB as one level,
	//so when going through a HUB, resetting the wad_score to 0 really means
    //keeping the wad_score as one level_score, since the wad_score picks up the
	//levelc_score
	//}
	//if (debugfile) fprintf (debugfile, "ghkPLEVEL2SCORE%d",p->level_score);
	...
	//ghk fix for maps endings like for scythe map10
	
	int do_sd_piststart	= p->health>0?sd_piststart:0;


	// Clears the entire inventory and gives back the defaults for starting a game
	if (resetinventory || do_sd_piststart) //ghk
	{
		...
		if (do_sd_piststart) //ghk reset player health
...
void G_PlayerReborn (int player)
{
	...
	int 		barrelcount; //ghk
	int			chasecam;
	int			wadscore; //ghk new var.
	int			levelscore; //ghk new var.
	BYTE		currclass;
	...
	wadscore = p->wad_score; //GHK ADDED
	barrelcount = p->barrelcount;


	//GHK: Check multiplayer flags, to see if player should
	//restart with current level score in CO-OP. Yes they should.
//GHK get the HUB info....
	...
	{
		mode = FINISH_SameHub;
		//p->level_score=p->wad_score; //ghk, for same hub, level_score matches wad_score
	}
	...
	p->wad_score=wadscore; //wminfo.plyr[player].wad_score;
	p->barrelcount=barrelcount; //ghk
	//GHK: Check multiplayer flags, to see if player should
	//restart with current level score in CO-OP. Yes they should.

	if(multiplayer){
		p->level_score=levelscore;
...
CVAR (Int, sdhcsavenum, 0, CVAR_NOSET|CVAR_ARCHIVE|CVAR_GLOBALCONFIG) //GHK
...
CUSTOM_CVAR (Int, sdhcsavecount, 32, CVAR_ARCHIVE|CVAR_GLOBALCONFIG) //ghk
{
	if (self < 0)
		self = 0;
	if (self > 64)
...
void G_DoAutoSave ()
{
	if(sd_global_hiscores&&sd_remote_hiscores_ok&&!multiplayer&&sd_online_level>1){
		// Keep up to four SD hardcore autosaves at a time
		UCVarValue num;
		const char *readableTime;
		int count = sdhcsavecount != 0 ? sdhcsavecount : 1;

		num.Int = (sdhcsavenum + 1) % count;
		sdhcsavenum.ForceSet (num, CVAR_Int);

		savegamefile = G_BuildSaveName ("hard", num.Int);

		readableTime = myasctime ();
		strcpy (savedescription, "Hardcore ");
