...
public:
	...
	static AActor *StaticSpawn (const PClass *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool nsFlag=false); //ghk, added noscore flag
...
// interaction info
	...
	FName	 SDParentPClass; //GHK
	fixed_t	 		origx,origy,origz; //GHK, used with A_CustomMissileSD for boss chain 'wall-spawn' fix

	//GHK: FOR SSHOTTY LAG FIX
	int				lastpnumdmg;	//target's reference to playernum who last shot it, reset on Tick()
	int				multidmgbuffer; //used on player to accumlate damage dealt out per tic, released on Tick()
	int				lastscoretic;    //use to stop multiple scores
...
inline AActor *Spawn (const PClass *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool nsFlag = false)
{
	return AActor::StaticSpawn (type, x, y, z, allowreplacement,nsFlag);
}

//GHK, Spawn SD No Scores
AActor *Spawn (const char *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool nsFlag = false);

//GHK, Spawn SD No Scores
//Actor *SpawnNS (const char *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement);
...
template<class T>
inline T *SpawnNS (fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement)
{
	return static_cast<T *>(AActor::StaticSpawn (RUNTIME_CLASS(T), x, y, z, allowreplacement,true));
}
