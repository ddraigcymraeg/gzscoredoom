...
//---------------------------------------------------------------------------
	...
	void UpdateFace ()
	{
		...
		//ghk
		if(ST_MUCHPAIN > CPlayer->damagecount)
			MuchFlag=0;
		...
		if (FacePriority < 8)
		{
				...
				&& CPlayer->attacker != CPlayer->mo)
			{
				...
				//ghk: if (FaceHealth != -9999 && FaceHealth - CPlayer->health > ST_MUCHPAIN)
				if(CPlayer->damagecount > ST_MUCHPAIN&&!MuchFlag)
				{
	...
	int OldMaxAmmo[4];
	int OldHealth;
	int MuchFlag; //ghk, fix for ouchface
	int OldArmor;
	int OldActiveAmmo;
	int OldFrags;
	int FaceHealth;
