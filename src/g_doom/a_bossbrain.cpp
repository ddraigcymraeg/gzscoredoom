...
void A_SpawnFly (AActor *self)
{
	...
	//GHK add support for Doom 1 as well.
	if(gameinfo.flags & GI_MAPxx){  //only for Doom2+
		...
		else if (r < 200) type = "Cacodemon";
		else			  type = "BaronOfHell";

	}
	if(PClass::FindClass ("SDSmartBomb")) //ghk add-on pack request to spawn powerups
		PClass::GetRandomMonster (PClass::FindClass ("ZombieMan"), 0, targ->x, targ->y, targ->z, false); 

	newmobj = Spawn (type, targ->x, targ->y, targ->z, ALLOW_REPLACE, true); //GHK, Changed from Spawn(..)
	if (newmobj != NULL)
	{
