...
bool P_MorphMonster (AActor *actor, const PClass *spawntype)
{
	...
	//GHK, dont morph if on a BOSS or SPECIAL level
	/*if ((level.flags & (LEVEL_CYBORGSPECIAL|
							LEVEL_SPIDERSPECIAL|LEVEL_BRUISERSPECIAL|LEVEL_MAP07SPECIAL)) != 0){
		C_MidPrint ("Porkelation Failed! Boss didnt allow it!");
...
bool P_UpdateMorphedMonster (AMorphedMonster *beast)
{
	...
	//GHK, pointsbunny is countaskill, so decrease total_monsters count to compensate
	//if not, cannot get 100% kills if the creature unmorphs
