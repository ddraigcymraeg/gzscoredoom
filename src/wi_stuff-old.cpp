...
void WI_LoadBackground(bool isenterpic)
{
	...
	if (!(gameskill == sk_nightmare)){
		...
		//if (consoleplayer == Net_Arbitrator){
			...
			GameConfig->SetHiScore(line,wbs->current,false);
			sprintf (line,"%d",plrs[0].wad_score);
			GameConfig->SetHiScore(line,wbs->current,true);
		//}
		}
	}	
}
...
void WI_initNetgameStats ()
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		cnt_levelscore[i]=cnt_wadscore[i]=cnt_kills[i] = cnt_items[i] = cnt_secret[i] = cnt_frags[i] = 0;
...
void WI_drawNetgameStats ()
{
	...
	if (gameinfo.gametype == GAME_Doom)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			//GHK Draw points
			x-=3.9*NG_SPACINGX;
			sprintf (line,"PTS:" TEXTCOLOR_GREEN" %d",plrs[i].level_score);	// Total score
			...
			x+=1.5*NG_SPACINGX;
			sprintf (line,"TOT.PTS:" TEXTCOLOR_BLUE" %d",plrs[i].wad_score);	// Total score
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
...
void WI_initStats ()
{
	...
	cnt_levelscore[0]=cnt_wadscore[0]=0;
	cnt_time = cnt_par = -1;
	cnt_pause = TICRATE;
...
void WI_updateStats ()
{
		...
		&& sp_state != 10)
	{
		...
		cnt_levelscore[0]=plrs[me].level_score;
		cnt_wadscore[0]=plrs[me].wad_score;
...
void WI_drawStats (void)
{
	...
	if (gameinfo.gametype == GAME_Doom)
	{
		...
		//GHK ADDED:
		screen->SetFont (BigFont);
		screen->DrawText (CR_RED, SP_STATSX, SP_STATSY+3*lh, "POINTS", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		//WI_drawNum (320 - SP_STATSX, SP_STATSY+3*lh, cnt_levelscore[0], 3, false);
		sprintf (line,"%d",plrs[me].level_score);	// Total score
		screen->DrawText (CR_RED, 320 - SP_STATSX-30, SP_STATSY+3*lh, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
			


		//GHK ADDED:
		screen->SetFont (BigFont);
		screen->DrawText (CR_RED, SP_STATSX, SP_STATSY+4*lh, "TOTAL POINTS", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		
		sprintf (line,"%d",plrs[me].wad_score);	// Total score
		screen->DrawText (CR_RED, 320 - SP_STATSX-30, SP_STATSY+4*lh, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				
		//WI_drawNum (320 - SP_STATSX, SP_STATSY+4*lh, cnt_wadscore[0], 3, false);
