...
#include "i_video.h"
#include "v_text.h"
#include "g_game.h" //ghk added
#include "g_level.h" //ghk added
#include "wi_stuff.h" //GHK: Added. For some reason couldnt access level_score thru players[]
#include "w_wad.h"
#include "s_sound.h"
#include "gstrings.h"
#include "doomstat.h"
...
void F_Drawer (void)
{
	...
	case 15:
		...
		//GHK added scores to bottom of end of game scene (ONLY SP atm):
	...
	//if(multiplayer){
	//GHK: Probably a good idea to remove this check in the future.
	for (int i=0 ; i<MAXPLAYERS ; i++)
		{
			if(playeringame[i]){
				playercount++;

				if((wminfo.plyr[i].level_score+wminfo.plyr[i].wad_score)>leaderScore){
					leaderScore=wminfo.plyr[i].level_score+wminfo.plyr[i].wad_score;
					leadernum = i;
					tied=false;
				}else if((wminfo.plyr[i].level_score + wminfo.plyr[i].wad_score)==leaderScore){
					tied=true;
				}
			}
	}
	...
	if (!(gameskill == (AllSkills.Size()-1))&&!sd_finale_didonce){
			//GHK:Server player decides hi scores.
			//Hi Scores only in SP right now.
		if(!multiplayer){
			//if (consoleplayer == Net_Arbitrator){
				//sd_finale_didonce=true; ghk move from here to bottom, out of !multiplayer
			//Printf("sd_finale_didonce %d\n",sd_finale_didonce);
				if(sd_global_hiscores&&sd_remote_hiscores_ok){
					...
					GameConfig->SetRemoteHiScores(line,line2,wminfo.current,plvlscr,pwadscr,true);
	...
	//}else{
		//didOnce=true; //write scores out on second pass. Is this ok for all the games?
	...
	if(SCREENWIDTH>800){
		...
		screen->DrawText(CR_RED, x, y-3*height, "ScoreDoom Stats", 
				DTA_CleanNoMove, false, TAG_DONE);
		...
		for (int i=0 ; i<MAXPLAYERS ; i++)
		{
			...
			if(playeringame[i]){
				...
				//sprintf (line,"TOTAL POINTS:" TEXTCOLOR_LIGHTBLUE " %d",wminfo.plyr[i].wad_score);	// Total score
				if(beatWadHiScore){
						sprintf (line,"TOT.PTS:" TEXTCOLOR_GOLD " %d",players[i].wad_score);	// Total score
				
				}else if(beatWadSilverHiScore){
						sprintf (line,"TOT.PTS:" TEXTCOLOR_GRAY " %d",players[i].wad_score);	// Total score
				}else if(beatWadBronzeHiScore){
						sprintf (line,"TOT.PTS:" TEXTCOLOR_DARKBROWN " %d",players[i].wad_score);	// Total score
				}else{
						sprintf (line,"TOT.PTS:" TEXTCOLOR_WHITE " %d",players[i].wad_score);	// Total score
				}
				//screen->SetFont(BigFont);
				//screen->DrawText (highlight, x, y, line, DTA_CleanNoMove, true, TAG_DONE);
				
				screen->DrawText (CR_RED, x, y, line, DTA_CleanNoMove, false,TAG_DONE);
				
				//screen->DrawText (highlight, x, y, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				//screen->DrawText (highlight, x, y, line, TAG_DONE);
				if(!(thiscluster->flags & CLUSTER_HUB)){
					y-=height;
					//GHK: For some reason couldnt access level-score thru players[]...
					//sprintf (line,"POINTS:" TEXTCOLOR_GREEN " %d",wminfo.plyr[i].level_score);	// Total score
					if(beatLvlHiScore){
							sprintf (line,"PTS:" TEXTCOLOR_GOLD " %d",wminfo.plyr[i].level_score);	// Total score
					}else if(beatLvlSilverHiScore){
						sprintf (line,"PTS:" TEXTCOLOR_GRAY " %d",wminfo.plyr[i].level_score);	// Total score
					}else if(beatLvlBronzeHiScore){
						sprintf (line,"PTS:" TEXTCOLOR_DARKBROWN " %d",wminfo.plyr[i].level_score);	// Total score					
					
					}else{
							sprintf (line,"PTS:" TEXTCOLOR_WHITE " %d",wminfo.plyr[i].level_score);	// Total score
					}
					//screen->DrawText (highlight, x, y, line, DTA_CleanNoMove, true, TAG_DONE);
					screen->DrawText (CR_RED, x, y, line, DTA_CleanNoMove, false,
						TAG_DONE);
				}
				
				//screen->DrawText (highlight, x, y, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				y-=height;
					if(multiplayer&&(leadernum==i)&&!tied){

						if(!sd_finale_didonce&&i==consoleplayer){
							S_Sound (CHAN_VOICE, "announcer/q3a/youwin", 1, ATTN_NORM);	

						//S_Sound (players[i].mo, CHAN_VOICE, "announcer/q3a/youwin", 1, ATTN_NONE);
						}
						sprintf (line,"" TEXTCOLOR_GOLD " %s",players[i].userinfo.netname);	// Total score
					}else{
						if(multiplayer&&(i==consoleplayer)&&!sd_finale_didonce&&!tied){
							S_Sound (CHAN_VOICE, "announcer/q3a/humiliation", 1, ATTN_NORM);
							//S_Sound (players[i].mo, CHAN_VOICE,"announcer/q3a/youwin", 1, ATTN_NONE);
						}
						sprintf (line,"" TEXTCOLOR_WHITE " %s",players[i].userinfo.netname);	// Total score
					
					}

				screen->DrawText (highlight, x, y, line, DTA_CleanNoMove, false,
				TAG_DONE);
				//screen->DrawText (highlight, x, y, line, DTA_CleanNoMove, true, TAG_DONE);
				//screen->DrawText (highlight, x, y, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		...
		//for small resolution 640x480, use a smaller font for 4 players.
		//a hack for now
		...
		if(playercount<4){
			...
			for (int i=0 ; i<MAXPLAYERS ; i++)
			{
				if(playeringame[i]){
					...
					if(!(thiscluster->flags & CLUSTER_HUB)){
						y-=height;
						//GHK: For some reason couldnt access level-score thru players[]...
						//sprintf (line,"PTS:" TEXTCOLOR_GREEN " %d",wminfo.plyr[i].level_score);	// Total score
						if(beatLvlHiScore){
							sprintf (line,"PTS:" TEXTCOLOR_GOLD " %d",wminfo.plyr[i].level_score);	// Total score
						}else if(beatLvlSilverHiScore){
							sprintf (line,"PTS:" TEXTCOLOR_GRAY " %d",wminfo.plyr[i].level_score);	// Total score
					...
					if(multiplayer&&(leadernum==i)&&!tied){
						...
						if(!sd_finale_didonce)
							S_Sound (CHAN_VOICE, "announcer/q3a/youwin", 1, ATTN_NORM);	
						
						sprintf (line,"" TEXTCOLOR_GOLD " %s",players[i].userinfo.netname);	// Total score
					}else{
						if(multiplayer&&!sd_finale_didonce&&!tied)
							S_Sound (CHAN_VOICE, "announcer/q3a/humiliation", 1, ATTN_NORM);
			...
			for (int i=0 ; i<MAXPLAYERS ; i++)
			{
				...
				if(playeringame[i]){
					//sprintf (line,"TOT.PTS:" TEXTCOLOR_LIGHTBLUE " %d",players[i].wad_score);	// Total score
					if(beatWadHiScore){
						sprintf (line,"TOT.PTS:" TEXTCOLOR_GOLD " %d",players[i].wad_score);	// Total score
					}else if(beatWadSilverHiScore){
						sprintf (line,"TOT.PTS:" TEXTCOLOR_GRAY " %d",players[i].wad_score);	// Total score
					...
					//sprintf (line,"" TEXTCOLOR_WHITE " %s",players[i].userinfo.netname);	// Total score
					screen->DrawText (highlight, x, y, line, TAG_DONE);
					//screscreen->DrawText (highlight, x, y, line, TAG_DONE);en->DrawText (highlight, x, y, line, DTA_CleanNoMove, false,
						//TAG_DONE);
					//screen->DrawText (highlight, x, y, line, DTA_CleanNoMove, true, TAG_DONE);
					//screen->DrawText (highlight, x, y, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
