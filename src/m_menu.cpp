...
//GHK ScoreDoom setup menu
	   void M_SDSetup ();
//static void M_SDSetupTicker ();
//static bool M_RemoteSaveSDPWDOK ();
static void M_SDSetupDrawer ();
static void M_EditSDURL (int choice);
static void M_EditSDPWD (int choice);
static void M_SDURLChanged (FSaveGameNode *dummy);
static void M_SDURLNotChanged ();
static void M_SDPWDChanged (FSaveGameNode *dummy);
static void M_SDPWDNotChanged ();
static void M_ChangeSDOnline (int choice);
static void M_ChangeSDBBAnnouncer (int choice);
static void M_ChangeSDNoInfight (int choice);
static void M_ChangeSDPistolStart (int choice);
...
EXTERN_CVAR (String,sd_hiscoreserver_pwd) //ghk
EXTERN_CVAR (String,sd_hiscoreserver_url) //ghk
EXTERN_CVAR (Bool, cl_bbannounce) //ghk
EXTERN_CVAR (Int,sd_global_hiscores) //ghk
EXTERN_CVAR (Int, sd_noinfight) //ghk
EXTERN_CVAR (Int, disableautosave) //ghk
...
static char		savegamestring2[16384]; //GHK
...
//GHK
static oldmenuitem_t SDSetupMenu[] =
{
	{ 2,0,'o',NULL,M_ChangeSDOnline, CR_UNTRANSLATED},
	{ 2,0,'u',NULL,M_EditSDURL, CR_UNTRANSLATED},
	{ 2,0,'p',NULL,M_EditSDPWD, CR_UNTRANSLATED},
	{ 2,0,'p',NULL,M_ChangeSDBBAnnouncer, CR_UNTRANSLATED},
	{ 2,0,'p',NULL,M_ChangeSDNoInfight, CR_UNTRANSLATED},
	{ 2,0,'p',NULL,M_ChangeSDPistolStart, CR_UNTRANSLATED}
};

static oldmenu_t SDSetupDef =
{
	countof(SDSetupMenu),
	SDSetupMenu,
	M_SDSetupDrawer,
				...
				if (file != NULL)
				{
					...
					else
					{
						...
						if (fread (sig, 1, 16, file) == 16)
						{
							...
							//if (strncmp (sig, "ZDOOMSAVE", 9) == 0) GHK
							if (strncmp (sig, "SDOOMSAVE", 9) == 0)
							{
							...
							else
							{
								...
								if (fread (title + 16, 1, SAVESTRINGSIZE-16, file) == SAVESTRINGSIZE-16 &&
									...
									strncmp (sig, "SDOOMSAVE", 9) == 0) //GHK strncmp (sig, "ZDOOMSAVE", 9) == 0)
								{
									addIt = true;
								}
							}
...
void M_SDSetup (void)
{
	OptionsActive = false;
	drawSkull = true;
	strcpy (savegamestring, sd_hiscoreserver_pwd);//ghk
	strcpy (savegamestring2, sd_hiscoreserver_url);
	M_DemoNoPlay = true;
	if (demoplayback)
		G_CheckDemoStatus ();
	M_SetupNextMenu (&SDSetupDef);
	/*
...
static void M_SDSetupDrawer ()
{
	...
	// Draw title
	const char * text = "ScoreDoom Setup"; //GStrings("MNU_PLAYERSETUP");
	...
	screen->SetFont (SmallFont);
	// Draw SD URL  box
	//screen->DrawText (label, SDSetupDef.x, SDSetupDef.y-LINEHEIGHT+yo, "Hi Score Server Settings", DTA_Clean, true, TAG_DONE);
	//M_DrawSaveLoadBorder (PSetupDef.x + 56, PSetupDef.y, MAXPLAYERNAME+1);


	// Draw SD URL  box
	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y+LINEHEIGHT+yo, "Hi Score Server URL", DTA_Clean, true, TAG_DONE);
	M_DrawSaveLoadBorder (SDSetupDef.x + 170, SDSetupDef.y+LINEHEIGHT, MAXPLAYERNAME+1);

	char * text2;
	if(strlen(savegamestring2)>15){
		text2=&savegamestring2[strlen(savegamestring2)-15];

	}else{
		text2=&savegamestring2[0];
	}
	screen->DrawText (CR_UNTRANSLATED, SDSetupDef.x + 170 + xo, SDSetupDef.y+LINEHEIGHT+yo, text2,
		DTA_Clean, true, TAG_DONE);
	// Draw cursor for SD URL box
	if (genStringEnter==3)
		screen->DrawText (CR_UNTRANSLATED,
			SDSetupDef.x + SmallFont->StringWidth(text2) + 170+xo,
			SDSetupDef.y +LINEHEIGHT+yo, underscore, DTA_Clean, true, TAG_DONE);



	// Draw SD PWD box
	//x = SmallFont->StringWidth ("Hi Score Server PWD") + 8 + PSetupDef.x;

	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y+LINEHEIGHT*2+yo, "Hi Score Server PWD", DTA_Clean, true, TAG_DONE);
	M_DrawSaveLoadBorder (SDSetupDef.x + 170, SDSetupDef.y+LINEHEIGHT*2, MAXPLAYERNAME+1);


	screen->DrawText (CR_UNTRANSLATED, SDSetupDef.x + 170 + xo, SDSetupDef.y+LINEHEIGHT*2+yo, savegamestring,
		DTA_Clean, true, TAG_DONE);
	// Draw cursor for SD PWD box
	if (genStringEnter==2)
		screen->DrawText (CR_UNTRANSLATED,
			SDSetupDef.x + SmallFont->StringWidth(savegamestring) + 170+xo,
			SDSetupDef.y + LINEHEIGHT*2 +yo, underscore, DTA_Clean, true, TAG_DONE);


	// Draw autoaim setting
	x = SmallFont->StringWidth ("Online Hi Scoring") + 8 + SDSetupDef.x;
	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y +yo, "Online Hi Scoring Mode", DTA_Clean, true, TAG_DONE);
	screen->DrawText (value,  SDSetupDef.x + 160, SDSetupDef.y + yo,
		sd_global_hiscores >0 ? (sd_global_hiscores == 1 ? "Regular" :"Hardcore") :"Off",DTA_Clean, true, TAG_DONE);

	// Draw autoaim setting
	x = SmallFont->StringWidth ("Announcer Voice") + 8 + SDSetupDef.x;
	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y + LINEHEIGHT*3+yo, "Announcer Voice", DTA_Clean, true, TAG_DONE);
	screen->DrawText (value,  SDSetupDef.x + 160, SDSetupDef.y + LINEHEIGHT*3+ yo,
		cl_bbannounce == true ? "On" :"Off",DTA_Clean, true, TAG_DONE);

	// Draw autoaim setting
	x = SmallFont->StringWidth ("No Infighting Mode") + 8 + SDSetupDef.x;
	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y + LINEHEIGHT*4+yo, "No Infighting Mode", DTA_Clean, true, TAG_DONE);
	screen->DrawText (value,  SDSetupDef.x + 160, SDSetupDef.y + LINEHEIGHT*4+ yo,
		sd_noinfight == 1 ? "On" :"Off",DTA_Clean, true, TAG_DONE);

		// Draw autoaim setting
	x = SmallFont->StringWidth ("Pistol Start Mode") + 8 + SDSetupDef.x;
	screen->DrawText (label, SDSetupDef.x, SDSetupDef.y + LINEHEIGHT*5+yo, "Pistol Start Mode", DTA_Clean, true, TAG_DONE);
	screen->DrawText (value,  SDSetupDef.x + 160, SDSetupDef.y + LINEHEIGHT*5+ yo,
		sd_pistolstart == 1 ? "On" :"Off",DTA_Clean, true, TAG_DONE);

	if(sd_global_hiscores >0){
		// Draw autoaim setting
		x = SmallFont->StringWidth ("Note: ScoreDoom requires a restart for\nchanges affecting online hi score modes") + 8 + SDSetupDef.x;
		screen->DrawText (value, SDSetupDef.x, SDSetupDef.y + LINEHEIGHT*6+yo, "Note: ScoreDoom requires a restart for\nchanges affecting online hi score modes", DTA_Clean, true, TAG_DONE);
...
{
	//static const float ranges[] = { 0, 0.25, 0.5, 1, 2, 3, 5000 };
	static const int ranges2[] = { 0, 1, 2 };

	int sd_online_scoring = sd_global_hiscores;
	int i;
	...
	if (!choice) {
		for (i = 2; i >= 1; i--)
		{
			if (sd_online_scoring >= ranges2[i])
			{
				sd_online_scoring = ranges2[i - 1];
				break;
			}
		}
		//sd_online_scoring =false;
	}else{
		for (i = 1; i >= 0; i--)
		{
			if (sd_online_scoring >= ranges2[i])
		...
		//sd_online_scoring =true;

	}
	sd_global_hiscores=sd_online_scoring;

	//GHK, need to disable online hi scoring
	sd_remote_hiscores_ok=false;

	//Printf ("**You will need to restart Scoredoom**\n");
	if(sd_global_hiscores&&!multiplayer){
			Printf (TEXTCOLOR_PURPLE"**You will need to restart Scoredoom**\n");
			Printf (TEXTCOLOR_PURPLE"**to use online hi scoring**\n");
			Printf (TEXTCOLOR_PURPLE"**ONLINE HI SCORING DISABLED**\n");
	}
...
{
	//static const float ranges[] = { 0, 0.25, 0.5, 1, 2, 3, 5000 };
	int sdPistolStart = sd_pistolstart;
...
static void M_EditSDPWD (int choice)
{
	...
	genStringEnter = 2;
	genStringEnd = M_SDPWDChanged;
	genStringCancel = M_SDPWDNotChanged;
...
static void M_SDPWDNotChanged ()
{
	strcpy (savegamestring, sd_hiscoreserver_pwd);
}

static void M_SDPWDChanged (FSaveGameNode *dummy)
{

	if(FGameConfigFile::M_RemoteSaveSDPWDOK(name, sd_hiscoreserver_pwd, savegamestring)){
		char command[SAVESTRINGSIZE+24];

		sprintf (command, "sd_hiscoreserver_pwd \"%s\"", savegamestring);
		C_DoCommand (command);

		C_DoCommand ("playsound misc/ldhscr");

	}else{
		C_DoCommand ("playsound misc/sdtout");
	}
...
static void M_EditSDURL (int choice)
{
	// we are going to be intercepting all chars
	genStringEnter = 3;
	genStringEnd = M_SDURLChanged;
	genStringCancel = M_SDURLNotChanged;
	genStringLen = 16383;

	saveSlot = 0;
	saveCharIndex = strlen (savegamestring2);
}
...
static void M_SDURLChanged (FSaveGameNode *dummy)
{
	char command[16384+8];

	sprintf (command, "sd_hiscoreserver_url \"%s\"", savegamestring2);
	C_DoCommand (command);
}
...
bool M_Responder (event_t *ev)
{
	...
	else if (ev->type == EV_GUI_Event && menuactive == MENU_On && !chatmodeon)
	{
		...
		else if (ev->subtype == EV_GUI_Char && genStringEnter)
		{
			...
			if(genStringEnter!=3){
				...
				if (saveCharIndex < genStringLen &&
				(genStringEnter==3 || (size_t)SmallFont->StringWidth (savegamestring2) < (genStringLen-1)*8))
				{

				savegamestring2[saveCharIndex] = ch;
				savegamestring2[++saveCharIndex] = 0;
	...
	if (genStringEnter)
	{
		...
		case '\b':
			...
			if(saveCharIndex==0) //GHK Stop crash with negative indexes.
				break;

			if (saveCharIndex > 0&&genStringLen!=16383) //GHK
			{
				saveCharIndex--;
				savegamestring[saveCharIndex] = 0;
			}else{
		...
		case '\r':
			...
			}else if (genStringEnter==3){ //ghk

				if(savegamestring2[0]){
					genStringEnter = 0;
					if (messageToPrint)
						M_ClearMenus ();
					genStringEnd (SelSaveGame);	// [RH] Function to call when enter is pressed
...
void M_Drawer ()
{
	...
	else if (menuactive != MENU_Off)
	{
		...
		else
		{
			...
			if (drawSkull)
			{
				if (currentMenu == &PSetupDef||currentMenu == &SDSetupDef) //GHK
				{
					// [RH] Use options menu cursor for the player setup menu.
					//GHK: and ScoreDoom setup
