...
void cht_DoCheat (player_t *player, int cheat)
{
	...
	//GHK: comment this below for cheats
	//return;
	//
	//GHK
...
//				player->mo->GiveDefaultInventory();
	...
	//GHK: add chtblip noise too, since messages can get flooded.
	//msg="NO CHEATS ALLOWED!";
	if (player == &players[consoleplayer]){
		Printf ("%s\n", msg);
	}else{
		Printf ("%s tried to cheat: %s\n", player->userinfo.netname, msg);
		S_Sound (player->mo,CHAN_VOICE, "misc/chtblp", 1, ATTN_NORM);
	}
...
void GiveSpawner (player_t *player, const PClass *type, int amount)
{
	...
	if (item != NULL)
	{
		...
		if (!item->TryPickup (player->mo))
		{
			...
			if (player->mo->CheckLocalView(consoleplayer)) //ghk play the pickup announcer
			{
				if (item->PickupAnnouncerEntry())
					S_Sound( CHAN_VOICE, item->PickupAnnouncerEntry(), 1, ATTN_NONE );
			}
		}
	}
}
