...
static int				cnt_barrels[MAXPLAYERS]; //ghk
static int				cnt_time;
static int				cnt_total_time;
static int				cnt_par;
static int				cnt_pause;
static bool				noautostartmap;
static int				cnt_levelscore[MAXPLAYERS]; //GHK new vars
static int				cnt_wadscore[MAXPLAYERS];

//GHK
static int  leadernum=0;
static bool tied = false;
static int  kleadernum=0;
static bool ktied = false;
static int  ileadernum=0;
static bool itied = false;
static int  bleadernum=0;
static bool btied = false;
static int  sleadernum=0;
static bool stied = false;

static bool beatLvlHiScore = false;
static bool beatWadHiScore = false;

static bool beatLvlSilverHiScore = false;
static bool beatWadSilverHiScore = false;

static bool beatLvlBronzeHiScore = false;
static bool beatWadBronzeHiScore = false;

static const char *prevLvlHiScore;
static const char *prevWadHiScore;
bool sd_wistuff_didonce=false;
bool sd_wistuff_didonce2=false;
...
void WI_LoadBackground(bool isenterpic)
{
	...
	//GHK Save hi scores here, per intermission/finale screen aftr a level is played
	//GameConfig = new FGameConfigFile;
	char line[64+10];
	char line2[64+10];


	//GHK Just deal with SP for now.
	//dont save if in nightmare mode, or player is turboing
	//turbo dealt with elsewhere
	//Printf("sd_wistuff_didonce %d\n",sd_wistuff_didonce);

	//if (debugfile) fprintf (debugfile, "sd_wistuff_didonce: %d",sd_wistuff_didonce);
			   
	//if (!(gameskill == sk_nightmare)){
	if (!(gameskill == (AllSkills.Size()-1))&&!sd_wistuff_didonce){
		//GHK:Server player decides hi scores.
		//Hi Scores only in SP right now.
		if(!multiplayer){
		//if (consoleplayer == Net_Arbitrator){
			sd_wistuff_didonce=true;
			beatLvlHiScore= false;
			beatWadHiScore = false;
			beatLvlSilverHiScore= false;
			beatWadSilverHiScore = false;
			beatLvlBronzeHiScore= false;
			beatWadBronzeHiScore = false;
			...
			if(sd_global_hiscores&&sd_remote_hiscores_ok){
				...
				sprintf(plvlscr, "%d", plrs[0].level_score);
				char pwadscr[64+10];
				sprintf(pwadscr, "%d", plrs[0].wad_score);


				GameConfig->SetRemoteHiScores(line,line2,wbs->current,plvlscr,pwadscr);

				if(!sd_remote_hiscores_ok){ //this global can get set in setremotehiscores above
					//revert to regular local hi scores
					sprintf (line,"%d",plrs[0].level_score);	
					beatLvlHiScore=GameConfig->SetHiScore(line,wbs->current,false);
					sprintf (line,"%d",plrs[0].wad_score);
					beatWadHiScore=GameConfig->SetHiScore(line,wbs->current,true);
				...
				//GHK everything ok!
					plrs[0].level_score=atoi(plvlscr);
					plrs[0].wad_score=atoi(pwadscr);
					...
					//used for awarding gold text..bit of hack since this code gets called twice
						...
						/*
						if(beatLvlHiScore||beatWadHiScore){
...
void WI_drawLF ()
{
	...
	if (y < NG_STATSY - screen->Font->GetHeight()*3/4)
	{
		...
		if (gameinfo.gametype == GAME_Doom) 
		{
			...
			//screen->DrawText(CR_RED, 160 - screen->Font->StringWidth("ScoreDoom Stats")/2, y+16, "ScoreDoom Stats", 
		...
		else 
		{
			...
			screen->DrawText(CR_WHITE, 160 - screen->Font->StringWidth("finished")/2, y-4, "finished", 
				DTA_Clean, true, TAG_DONE);
			//ghk
			//screen->DrawText(CR_RED, 160 - screen->Font->StringWidth("ScoreDoom Stats")/2, y+12, "ScoreDoom Stats", 
				//DTA_Clean, true, TAG_DONE);
			screen->SetFont(SmallFont);
		}
	}
...
void WI_initNetgameStats ()
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		cnt_kills[i] = cnt_items[i] = cnt_secret[i] = cnt_frags[i] = cnt_barrels[i] = 0;
...
void WI_updateNetgameStats ()
{
	...
	/*
	use: S_Sound (players[consoleplayer].mo, CHAN_VOICE, "*death", 1, ATTN_NONE); instead?
	if(!sd_wistuff_didonce2){ //ghk
		int leaderScore=0;
		//ghk
		//reset
		leadernum=0;
		...
		for (i=0 ; i<MAXPLAYERS ; i++)
		{
			...
			if((plrs[i].level_score+plrs[i].wad_score)>leaderScore){
				leaderScore=plrs[i].level_score+plrs[i].wad_score;
				leadernum = i;
				tied=false;
			}else if((plrs[i].level_score + plrs[i].wad_score)==leaderScore){
				tied=true;
			}
			...
			cnt_levelscore[i] = plrs[i].level_score; //GHK NEW VARS
			cnt_wadscore[i]= plrs[i].wad_score;
		...
		for (i=0 ; i<MAXPLAYERS ; i++)
		{
			...
			sd_wistuff_didonce2=true;
	}
	*/

	if (1 && ng_state != 10) //ghk force acceleratestage here (acceleratestage && ng_state != 10)
	{
		...
		//ghk: items, kills, barrels and secrets counts
		int kleaderScore=0;
		int ileaderScore=0;
		int bleaderScore=0;
		int sleaderScore=0;
		...
		//reset
		leadernum=0;
		kleadernum=bleadernum=sleadernum=ileadernum=0;
		ktied=btied=stied=itied=0;
		...
		for (i=0 ; i<MAXPLAYERS ; i++)
		{
			...
			if(!tied){
				if(leadernum==me)
					S_Sound (CHAN_VOICE, "announcer/q3a/youwin", 1, ATTN_NORM);		
				else
					S_Sound (CHAN_VOICE, "announcer/q3a/humiliation", 1, ATTN_NORM);
			}	
			
			//ghk: items, kills, barrels and secrets counts
			if(plrs[i].skills>kleaderScore){
				kleaderScore=plrs[i].skills;
				kleadernum = i;
				ktied=false;
			}else if(plrs[i].skills==kleaderScore){
				ktied=true;
			}

			if(plrs[i].sbarrels >bleaderScore){
				bleaderScore=plrs[i].sbarrels;
				bleadernum = i;
				btied=false;
			}else if(plrs[i].sbarrels==bleaderScore){
				btied=true;
			}

			if(plrs[i].sitems >ileaderScore){
				ileaderScore=plrs[i].sitems;
				ileadernum = i;
				itied=false;
			}else if(plrs[i].sitems==ileaderScore){
				itied=true;
			}

			if(plrs[i].ssecret >sleaderScore){
				sleaderScore=plrs[i].ssecret;
				sleadernum = i;
				stied=false;
			}else if(plrs[i].ssecret==sleaderScore){
				stied=true;
			}
...
{
	int i, x, y;
	//GHK: new vars for drawing level_score, wad_score
	...
	if (gameinfo.gametype == GAME_Doom)
	{
		...
		screen->DrawText(CR_RED, 160 - screen->Font->StringWidth("ScoreDoom Stats")/2, NG_STATSY-lh, "ScoreDoom Stats", 
				DTA_Clean, true, TAG_DONE);
		screen->DrawTexture (kills, NG_STATSX+NG_SPACINGX-kills->GetWidth()-12, NG_STATSY, DTA_Clean, true, TAG_DONE);
		screen->DrawTexture (items, NG_STATSX+2*NG_SPACINGX-items->GetWidth()-12, NG_STATSY, DTA_Clean, true, TAG_DONE);
		screen->DrawTexture (secret, NG_STATSX+3*NG_SPACINGX-secret->GetWidth()-12, NG_STATSY, DTA_Clean, true, TAG_DONE);
		//ghk
		screen->DrawText (CR_RED,  NG_STATSX+4*NG_SPACINGX-secret->GetWidth()-12, NG_STATSY, "BRLS", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		if (dofrags)
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			WI_drawPercent (x-pwidth-12, y+10, cnt_kills[i],wbs->maxkills); 	
			
			//ghk kills
			if(i==kleadernum&&!ktied?true:false){
				sprintf (line,TEXTCOLOR_GOLD"+%d",plrs[i].skills*10>100?plrs[i].skills*10:100 );
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
				screen->SetFont (SmallFont);
				screen->DrawText (highlight, x-2*pwidth-2*12, y+4, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				screen->SetFont (BigFont);
			}

			x += NG_SPACINGX;
			WI_drawPercent (x-pwidth-12, y+10, cnt_items[i],wbs->maxitems);	
			
			//ghk itemss
			if(i==ileadernum&&!itied?true:false){
				sprintf (line,TEXTCOLOR_GOLD"+%d",plrs[i].sitems*10>100?plrs[i].sitems*10:100 );
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
				screen->SetFont (SmallFont);
				screen->DrawText (highlight, x-2*pwidth-2*12, y+4, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				screen->SetFont (BigFont);
			...
			x += NG_SPACINGX;
			WI_drawPercent (x-pwidth-12, y+10, cnt_secret[i],wbs->maxsecret);	

			//ghk secrets
			if(i==sleadernum&&!stied?true:false){ 
				sprintf (line,TEXTCOLOR_GOLD"+%d",plrs[i].ssecret*10>100?plrs[i].ssecret*10:100 );
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
				screen->SetFont (SmallFont);
				screen->DrawText (highlight, x-2*pwidth-2*12, y+4, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				screen->SetFont (BigFont);
			...
			x += NG_SPACINGX;
			WI_drawPercent (x-pwidth-12, y+10, cnt_barrels[i],wbs->maxbarrels);	
			
			//ghk barrels
			if(i==bleadernum&&!btied?true:false){  
				sprintf (line,TEXTCOLOR_GOLD"+%d",plrs[i].sbarrels*10>100?plrs[i].sbarrels*10:100 );
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
				screen->SetFont (SmallFont);
				screen->DrawText (highlight, x-2*pwidth-2*12, y+4, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
				screen->SetFont (BigFont);
			...
			x-=4.9*NG_SPACINGX;
			screen->SetFont (SmallFont);
			sprintf (line,"PTS:" TEXTCOLOR_WHITE" %d",plrs[i].level_score);	// Total score
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
			
			screen->DrawText (highlight, x, y+25, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
			

			x+=1.5*NG_SPACINGX;
			if((leadernum==i)&&!tied){
				sprintf (line,"TOT.PTS:" TEXTCOLOR_GOLD" %d",plrs[i].wad_score);	// Total score
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
			
				screen->DrawText (highlight, x, y+25, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
			}else{
				sprintf (line,"TOT.PTS:" TEXTCOLOR_WHITE" %d",plrs[i].wad_score);	// Total score
					//screen->DrawText (highlight, x, y+45, line, DTA_CleanNoMove, true, TAG_DONE);
...
void WI_initStats ()
{
	...
	acceleratestage = 0;
	sp_state = 1;
	cnt_kills[0] = cnt_items[0] = cnt_secret[0] = cnt_barrels[0]=-1;
	cnt_time = cnt_par = -1;
	cnt_pause = TICRATE;
...
{	
	...
	//GHK
	if(!sd_wistuff_didonce2){
		DoVoiceAnnounceIntermission(beatLvlHiScore||beatWadHiScore);
		sd_wistuff_didonce2=true;
	}
	
	if ((gameinfo.gametype != GAME_Doom || 1) //ghk, force past stat counters... ||acceleratestage)
		&& sp_state != 10)
	{
		if (acceleratestage)
		{
		...
		cnt_barrels[0] = plrs[me].sbarrels;
...
void WI_drawStats (void)
{
	...
	if (gameinfo.gametype == GAME_Doom)
	{
		...
		screen->DrawText(CR_RED, 160 - screen->Font->StringWidth("ScoreDoom Stats")/2, SP_STATSY-lh, "ScoreDoom Stats", 
				DTA_Clean, true, TAG_DONE);
		...
		//ghk
		screen->DrawText (CR_RED, SP_STATSX, SP_STATSY+3*lh, "BARRELS", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		WI_drawPercent(320 - SP_STATSX, SP_STATSY+3*lh, cnt_barrels[0], wbs->maxbarrels);
		
		//GHK ADDED TO remove level score from end of hub intermission screen...
		...
		//GHK ADDED:
		//if (debugfile) fprintf (debugfile,  "ghkhub%d", thiscluster->flags & CLUSTER_HUB);

		if (!(thiscluster->flags & CLUSTER_HUB)){//(mode==FINISH_NoHub){
			screen->SetFont (BigFont);
			screen->DrawText (CR_RED, SP_STATSX, SP_STATSY+4*lh, "LEVEL SCORE", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
			//WI_drawNum (320 - SP_STATSX, SP_STATSY+3*lh, cnt_levelscore[0], 3, false);
			
			if(beatLvlHiScore){
				sprintf (line,TEXTCOLOR_GOLD"%d",plrs[me].level_score);	// level score
			}else if(beatLvlSilverHiScore){
				sprintf (line,TEXTCOLOR_GRAY"%d",plrs[me].level_score);	// level score
			}else if(beatLvlBronzeHiScore){
				sprintf (line,TEXTCOLOR_DARKBROWN"%d",plrs[me].level_score);	// level score
			}else{
				sprintf (line,"%d",plrs[me].level_score);
			}
			
			screen->DrawText (CR_RED, 320 - SP_STATSX-30, SP_STATSY+4*lh, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		}	

		//GHK ADDED:
		screen->SetFont (BigFont);
		screen->DrawText (CR_RED, SP_STATSX, SP_STATSY+5*lh, "WAD/EPI SCORE", DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
		if(beatWadHiScore){
			sprintf (line,TEXTCOLOR_GOLD"%d",plrs[me].wad_score);	// Total score
		}else if(beatWadSilverHiScore){
			sprintf (line,TEXTCOLOR_GRAY"%d",plrs[me].wad_score);	// Total score
		}else if(beatWadBronzeHiScore){
			sprintf (line,TEXTCOLOR_DARKBROWN"%d",plrs[me].wad_score);	// Total score
		}else{
			sprintf (line,"%d",plrs[me].wad_score);
		}
		screen->DrawText (CR_RED, 320 - SP_STATSX-30, SP_STATSY+5*lh, line, DTA_Clean, true, DTA_Shadow, true, TAG_DONE);
...
void WI_loadData(void)
{
	...
	sd_wistuff_didonce=false; //ghk
	sd_wistuff_didonce2=false;
