...
// memset it all to 0.
//GHK: added level_score & wad_score
player_s::player_s()
: mo(0),
  playerstate(0),
  cls(0),
...
void APlayerPawn::AddInventory (AInventory *item)
{
	...
	if (!deathmatch){
		//GHK: SHOULD MOVE TO A_KEYS? FOR PERFORMANCE?
		int spwnid = item->GetClass()->ActorInfo->SpawnID;

		//hack for hacx, since card keys are spwnid=0
		//add support for other spwnids, just in case
		if(spwnid==0){
			const char * testvar = item->GetClass()->TypeName.GetChars();
			if(strnicmp(testvar,"RedCard",7)==0||strnicmp(testvar,"KeyGreen",8)==0){
				spwnid=86;
			}else if(strnicmp(testvar,"BlueCard",8)==0||strnicmp(testvar,"KeyBlue",7)==0){
				spwnid=85;

			}else if(strnicmp(testvar,"YellowCard",10)==0||strnicmp(testvar,"KeyYellow",9)==0){
				spwnid=87;
			}

			if(strnicmp(testvar,"BlueSkull",9)==0){
				spwnid=90;
			}else if(strnicmp(testvar,"RedSkull",8)==0){
				spwnid=89;

			}else if(strnicmp(testvar,"YellowSkull",11)==0){
				spwnid=88;
			}
		}


		if(spwnid>84&&spwnid<91){
			int keyBonus = 250;
			bool keyScore = false;
			...
			if (gameinfo.gametype == GAME_Heretic){
				...
				//RedKey
						case 86:
							if(player->mo->FindInventory(PClass::FindClass ("KeyGreen"))==NULL&&!level.rkeyfound){
								//level.info->
								player->level_score+=keyBonus;
								...
								level.rkeyfound=!level.rkeyfound;
						...
						//BlueKey
						case 85:
							if(player->mo->FindInventory(PClass::FindClass ("KeyBlue"))==NULL&&!level.bkeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
								...
								level.bkeyfound=!level.bkeyfound;
								keyScore = true;
							}
							break;
						//YellowKey
						case 87:
							if(player->mo->FindInventory(PClass::FindClass ("KeyYellow"))==NULL&&!level.ykeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
								...
								level.ykeyfound=!level.ykeyfound;
								keyScore = true;
							}
							break;
				}
				...
				switch (spwnid){
						//RedKey
						case 86:
							if(player->mo->FindInventory(PClass::FindClass ("RedCard"))==NULL&&!level.rkeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
						...
						//BlueKey
						case 85:
							if(player->mo->FindInventory(PClass::FindClass ("BlueCard"))==NULL&&!level.bkeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
						...
						//YellowKey
						case 87:
							if(player->mo->FindInventory(PClass::FindClass ("YellowCard"))==NULL&&!level.ykeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
						...
						//BlueSkull
						case 90:
							if(player->mo->FindInventory(PClass::FindClass ("BlueSkull"))==NULL&&!level.bskeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
								...
								level.bskeyfound=!level.bskeyfound;
								keyScore = true;
							}
							break;
						//RedSkull
						case 89:
							if(player->mo->FindInventory(PClass::FindClass ("RedSkull"))==NULL&&!level.rskeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
								...
								level.rskeyfound=!level.rskeyfound;
								keyScore = true;
							}
							break;
						//YellowSkull
						case 88:
							if(player->mo->FindInventory(PClass::FindClass ("YellowSkull"))==NULL&&!level.yskeyfound){
								//level.info->
								player->level_score+=keyBonus;
								if(player->mo->CheckLocalView (consoleplayer)){
									char levelscore[40];
								...
								level.yskeyfound=!level.yskeyfound;
								keyScore = true;
							}
						break;
			...
			if(keyScore&&player->mo->CheckLocalView (consoleplayer))
			...
			{
				key = static_cast<AKey*>(item);
				&key->KeyNumber;
					Printf (PRINT_MEDIUM, "KeyNumber: %d", &key->KeyNumber);
	...
	else
	{
		...
		if (cmd->ucmd.buttons & BT_JUMP)
		{
			...
			else if (!(dmflags & DF_NO_JUMP) && onground && !player->jumpTics)
			{
				...
				// GHK If the player has the high jump power, double his jump velocity.
	...
	if (!(player->cheats & CF_PREDICTING))
	{
		...
		if (( level.time & 31 ) == 0 && ( player->cheats & CF_REGENERATION ) && ( player->health ))
		{
			if ( P_GiveBody( player->mo, 5 ))
			{
				...
				S_Sound(player->mo, CHAN_ITEM, "ghk/qregen", 1, ATTN_NORM ); //ghk
			}
		}
...
//GHK, Used to read back level_score when re-entering a Hub Level
void player_s::SerializeSDHubLvlScore (FArchive &arc)
{
...
void player_s::Serialize (FArchive &arc)
{
	...
	arc << cls
		...
		<< accuracy << stamina
		<< LogText
		<<level_score<<wad_score<<barrelcount; //GHK;
