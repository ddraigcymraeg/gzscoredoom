...
void FWadCollection::RenameSprites (int startlump)
{
		...
		++i)
	{
		...
		//GHK: Enforce Non-Doom game sprite renaming.
		//Some Heretic Mods may be affected. If complaints happen,
		//look for the proper fix, which I couldnt seem to find. Scoredoom doesnt officially
		//support Hexen or Strife.
		if (true)
		{
		...
		if (gameinfo.gametype != GAME_Doom)
		{
			...
			//GHK: Since I support Heretic also, do a 'hack' fix for IronLich:
			//if (*(DWORD *)LumpInfo[i].name == MAKE_ID('H','E','A','D'))
			//{
				//*(DWORD *)LumpInfo[i].name = MAKE_ID('L','I','C','H');
			//}
		}
	}
}
...
const char *FWadCollection::GetWadFullName (int wadnum) const
{
	...
	//GHK: if (debugfile) fprintf (debugfile, "wadfullname:%s",Wads[wadnum]->Name);
